...
const APP_VERSION = window.APP_BUILD || "V10.0.0";

const TARGET_RATE = 16000;
const SILENCE_MS = 2500;
const MIN_SPEECH_MS = 700;
const MAX_SEGMENT_MS = 26000;
...
function processQueue() {
  ...
  workerBusy = true;
  updateQueueText();
  setStatus(`正在用Whisper转写第${task.id}个问题`, "busy");
...
function createWorker() {
  ...
  worker = new Worker("./whisper-worker-v10.js?v=10.0.0", { type: "module" });
  ...
  worker.addEventListener("message", event => {
    ...
    if (message.type === "progress") {
      ...
      if (message.stage === "load") {
        ...
        $("progressText").textContent = "加载失败。Small占用较大时，请选择Whisper Base后重新加载。";
      }
      processQueue();
    }
  });
